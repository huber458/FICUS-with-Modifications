...
"""
spec_name, z_spec = str(sys.argv[1]), float(sys.argv[2]);#str(sys.argv[1]), np.float(sys.argv[2]);
...
#####MASON ADDITION. This is the DLA function
def gothroughfit(column_density,wavelength, redshift, doppler_parameter,spectrum,z):
    ...
    cutoff=(z+1)*1215.67e-10*10**6#10#6

    spectrum=np.where(shifted<cutoff,0,spectrum)#if the velocity is below 0, remain at 0 to avoid any nan issues
...
def ficus(path, spec_name, plot_mode, ssp_models, Zarray, att_law, wave_range, z_spec, wave_norm, R_obs, nsim):
    ...
    """ 
    params_array = np.zeros((10*len(Zarray)+1+3+1+1,nsim));####MASON ADDED a +1 for N AND another +1 for xH1
    ...
    for ns in np.arange(0,nsim,1):
        ...
        params.add('ebv', min=0., max=0.5);

        ####MASON ADDITION, Converting to MCMC
        params.add('N',  min=18., max=22.5);#Feel free to play with column density bounds here
        params.add('xH1',  min=0, max=1.0);###SHOULD BE 0 to 1
        print(len(params),"length")
        ndim=23
        nwalkers=50#On my computer needs to be at least 2*ndim
        p0 = np.random.rand(nwalkers,ndim)#Initializing random starts for all the walkers

        p0[:,0] = p0[:,0] #These are just the light fraction parameters for bookkeeping
        p0[:,1] = p0[:,1] 
        p0[:,2] = p0[:,2] 
        p0[:,3] = p0[:,3] 
        p0[:,4] = p0[:,4] 
        ...
        f = open("smoothfixedz18"+str(spec_name)+"v004.txt", "a")#I decided to save the chains as a text file. Probably not the most effective method! But you can read the chains out of the file.

        fit_ = minimize(residuals, params=params, args=(wave, custom_lib, att_law, flux_normSIM, err_normSIM,R_mod, R_obs, wave_norm,f), method='emcee',pos=p0,steps=10000, nwalkers=nwalkers, nan_policy='omit', scale_covar=True, reduce_fcn='neglogcauchy');

        f.close()#Close the file. I then just use the txt file I created to read out all parameters
        # reduced chi-2
        chi2 = fit_.redchi;
        ...
        if att_law == 'r16':
            ...
            kl = SMC(wave);
        hRspec = ssp2obs(fit_.params, wave, custom_lib_orig, att_law,R_mod, R_obs, wave_norm);
...
#         sed_hRmodel.append([wl_model,hRmodel*10**(0.4*kl*params_array[-1,ns]),hRmodel]);
        ...
        if att_law == 'r16':
            ...
            kl = SMC(wl_full);
        lRspec = ssp2obs(fit_.params, wl_full, models_full, att_law,R_mod, R_obs, wave_norm, fullSED=True);
    ...
    """ 
    ######MASON MAKING CHANGES HERE.  JUST IGNORE EVERYTHING BELOW HERE NOW######

    minchi=0#np.argmin(params_array[0])
    print('just 0',params_array[0])
    print('0 comma colon',params_array[0,:])
#    print(np.nanmedian(params_array, axis=1)-np.nanquantile(params_array,0.16, axis=1))
    params_output = np.c_[params_array[:,minchi],params_array[:,minchi]-np.nanquantile(params_array,0.16, axis=1),np.nanquantile(params_array,0.84, axis=1)-params_array[:,minchi]];
###    params_output = np.c_[np.nanmedian(params_array, axis=1), np.nanstd(params_array, axis=0)];
    SEDparams_output = np.c_[SEDparams[:,minchi], np.nanstd(SEDparams, axis=1)];
...
#     np.save(path+'/outputs/%s_ficus_OutputFiles/%s_ficusMC_hRSEDfull.npy' %(now.strftime('%Y%m%d'), spec_name), np.array(sed_hRmodel));
    ...
    if plot_mode == 'yes':
        ...
        ebv_def = params_output[-3,:];######MASON CHANGED THIS TO -2 and added the next line. NOW -3 with the addition of xH1
        N_def = params_output[-2,:];
        print('Mason Fiddling',chi2_def)
        print('Mason Fiddling',minchi)
#        print('Mason Fiddling',sed_model[,2])
        print('Mason Fiddling',np.array(sed_model)[minchi,2])
        xH1_def = params_output[-1,:];        
        ...
        ######MASON ADDED N_def above and BELOW##minchi#np.array(sed_model)[0,2],np.array(sed_model)[minchi,2]
     #   ficus_plot(pdf_file, spec_name, z_spec, wave, custom_data, custom_error, normID, mask_array, att_law, ebv_def,N_def,xH1_def, np.array(sed_model)[minchi,2], agew_def, Zw_def, agesws_def, age_array, Zarray, Z_set, chi2_def);
      #  pdf_file.close();

      #  pdf_file = PdfPages(path+'/outputs/%s_ficus_OutputFiles/%stest39.pdf' %(now.strftime('%Y%m%d'), spec_name));
        ficus_plot(pdf_file, spec_name, z_spec, wave, custom_data, custom_error, normID, mask_array, att_law, ebv_def,N_def,xH1_def, np.array(sed_model)[0,2], agew_def, Zw_def, agesws_def, age_array, Zarray, Z_set, chi2_def);
        pdf_file.close();
      #  pdf_file = PdfPages(path+'/outputs/%s_ficus_OutputFiles/%stest3.pdf' %(now.strftime('%Y%m%d'), spec_name));
      #  ficus_plot(pdf_file, spec_name, z_spec, wave, custom_data, custom_error, normID, mask_array, att_law, ebv_def,N_def,xH1_def, np.array(sed_model)[3,2], agew_def, Zw_def, agesws_def, age_array, Zarray, Z_set, chi2_def);
    #    pdf_file.close();
...
fmt='  '.join(['%+12.5e'] + ['%+12.5e'] + ['%+12.5e']),
header = '  '.join(['%-12s']+ ['%-12s']+ ['%-12s']) %('# param', 'param.errup', 'param.errlo'), 
delimiter='\t', 
